...
def calculate_performance_ranking(prices_dataset, current_time, lookback_days_param):
    """Calculate performance ranking based on individual backtests."""
    performance_dict = {}
    ...
    for symbol in TRADING_SYMBOLS.keys():
        try:
            ...
            backtest_days = min(lookback_days_param, 30)  # Limit to 30 days for speed
            params = best_params_data.get(symbol, {}).get('best_params', get_default_params())
            ...
            print(f"{symbol}: {performance:.2f}% (Backtest)")
    ...
    if performance_dict:
        ...
        perf_df['rank'] = perf_df['performance'].rank(
            ascending=False)  # Rank in descending order (best performance gets rank 1)
